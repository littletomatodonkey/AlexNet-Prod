...
使用下面的命令下载inference模型到`AlexNet_paddle`目录，

```bash
wget https://paddle-model-ecology.bj.bcebos.com/model/alexnet_reprod/alexnet_infer.tar
# 您也可以将inference模型放在当前文件夹，压缩后上传至github，然后使用下面的命令解压
tar -xf alexnet_infer.tar
```
...
```bash
bash test_tipc/test_serving.sh ./test_tipc/configs/AlexNet/model_linux_gpu_normal_normal_serving_python_linux_gpu_cpu.txt
```  
